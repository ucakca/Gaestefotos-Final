...
router.post('/apply', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Feature gate: check package + host config + device
    try {
      await assertAiFeatureAllowed(eventId, 'style_transfer');
    } catch (gateErr: any) {
      return res.status(gateErr.httpStatus || 403).json({ error: gateErr.message, code: gateErr.code });
    }

    // Energy check
    const energyResult = await checkAndSpendEnergy(req, eventId, 'style_transfer');
