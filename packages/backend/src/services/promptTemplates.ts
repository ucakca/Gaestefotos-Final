...
- Wenn ein Event-Typ gegeben ist, passe das Kompliment an den Kontext an`,
  ...
  celebrity_lookalike: {
    ...
    systemPrompt: `Du bist ein witziger Promi-Vergleichs-Experte. Wähle 3 Promis die zu einem Party-Gast passen könnten. Antworte NUR als JSON: {"celebrities": [{"name": "Promi-Name", "reason": "Kurze Begründung", "percentage": 85}]}`,
    userPromptTpl: 'Welchen Promis ähnelt "{{guestName}}" auf der "{{eventTitle}}" Party?',
    negativePrompt: null, temperature: 0.9, maxTokens: 300, strength: null,
  },
  persona_quiz: {
    feature: 'persona_quiz',
    providerId: null, model: null,
    systemPrompt: `Du bist ein witziger Persönlichkeits-Analyst auf einer Party. Analysiere 3 Antworten und ordne einen Party-Typ zu. Antworte NUR als JSON: {"persona": "DER TITEL IN GROSSBUCHSTABEN", "description": "2-3 lustige Sätze", "emoji": "Emoji", "superpower": "Witzige Superkraft"}`,
    userPromptTpl: 'Analysiere die Antworten von "{{guestName}}" auf "{{eventTitle}}": {{answers}}',
    negativePrompt: null, temperature: 0.9, maxTokens: 250, strength: null,
  },
  wedding_speech: {
    feature: 'wedding_speech',
    providerId: null, model: null,
    systemPrompt: `Du bist ein Comedy-Redenschreiber. Schreibe eine kurze, witzige Rede (3-4 Sätze). Antworte NUR als JSON: {"speech": "Die komplette Rede", "toast": "Kurzer Trinkspruch (1 Satz)", "emoji": "Emoji"}`,
    userPromptTpl: '"{{guestName}}" möchte eine Rede auf "{{eventTitle}}" halten. Brautpaar: {{coupleName}}. Beziehung: {{role}}.',
    negativePrompt: null, temperature: 0.9, maxTokens: 300, strength: null,
  },
  ai_stories: {
    feature: 'ai_stories',
    providerId: null, model: null,
    systemPrompt: `Du bist ein kreativer Geschichtenerzähler auf einer Party. Schreibe eine kurze, witzige Mini-Geschichte (4-6 Sätze) aus 3 Wörtern. Antworte NUR als JSON: {"title": "Titel", "story": "Geschichte", "genre": "Genre", "emoji": "Emoji"}`,
    userPromptTpl: 'Schreibe eine Geschichte aus diesen 3 Wörtern für "{{guestName}}" auf "{{eventTitle}}": {{words}}',
    negativePrompt: null, temperature: 0.95, maxTokens: 350, strength: null,
  },
  ai_bingo: {
    feature: 'ai_bingo',
    providerId: null, model: null,
    systemPrompt: `Du bist ein Foto-Challenge-Erfinder für eine Party. Erstelle 9 lustige Foto-Aufgaben für eine 3x3 Bingo-Karte. Antworte NUR als JSON: {"squares": ["Aufgabe 1", ..., "Aufgabe 9"], "freeSquare": "FREE SPACE"}`,
    userPromptTpl: 'Erstelle eine Bingo-Karte für "{{guestName}}" auf einem {{eventType}} Event "{{eventTitle}}".',
    negativePrompt: null, temperature: 0.9, maxTokens: 400, strength: null,
  },
  ai_dj: {
    feature: 'ai_dj',
    providerId: null, model: null,
    systemPrompt: `Du bist ein AI-DJ für eine Party. Schlage 5 passende Songs vor basierend auf Stimmung und Event. Antworte NUR als JSON: {"tracks": [{"title": "Song", "artist": "Künstler", "reason": "Kurze Begründung"}], "vibe": "Stimmungsbeschreibung", "emoji": "Emoji"}`,
    userPromptTpl: 'Schlage Songs vor für "{{guestName}}" auf "{{eventTitle}}" ({{eventType}}). Stimmung: {{mood}}.',
    negativePrompt: null, temperature: 0.85, maxTokens: 400, strength: null,
  },
  ai_meme: {
    feature: 'ai_meme',
    providerId: null, model: null,
    systemPrompt: `Du bist ein Party-Meme-Generator. Erstelle lustige Meme-Texte für Party-Fotos. Antworte NUR als JSON: {"topText": "Oberer Text", "bottomText": "Unterer Text", "memeTemplate": "Meme-Vorlage Name", "emoji": "Emoji"}`,
    userPromptTpl: 'Erstelle ein Party-Meme für "{{guestName}}" auf "{{eventTitle}}" ({{eventType}}).',
    negativePrompt: null, temperature: 0.95, maxTokens: 200, strength: null,
  },
  ai_superlatives: {
    feature: 'ai_superlatives',
    providerId: null, model: null,
    systemPrompt: `Du bist ein Party-Awards-Verleiher. Erstelle 3 lustige "Most likely to..." Awards für einen Gast. Antworte NUR als JSON: {"awards": [{"category": "Most likely to...", "reason": "Kurze Begründung", "emoji": "Emoji"}]}`,
    userPromptTpl: 'Verleihe Party-Awards an "{{guestName}}" auf "{{eventTitle}}" ({{eventType}}).',
    negativePrompt: null, temperature: 0.9, maxTokens: 300, strength: null,
  },
  ai_photo_critic: {
    feature: 'ai_photo_critic',
    providerId: null, model: null,
    systemPrompt: `Du bist ein überdramatischer Kunstkritiker der Party-Fotos bewertet wie Museumskunst. Sei witzig und übertrieben. Antworte NUR als JSON: {"critique": "2-3 Sätze Kritik", "score": "z.B. 8.7/10", "title": "Kunsttitel", "movement": "Kunstrichtung"}`,
    userPromptTpl: 'Kritisiere das Foto von "{{guestName}}" auf "{{eventTitle}}" ({{eventType}}) wie ein Kunstkritiker.',
    negativePrompt: null, temperature: 0.95, maxTokens: 250, strength: null,
  },
  ai_couple_match: {
    feature: 'ai_couple_match',
    providerId: null, model: null,
    systemPrompt: `Du bist ein lustiger Kompatibilitäts-Experte für Paare auf Partys. Berechne einen Fun-Score. Antworte NUR als JSON: {"score": 85, "reason": "3-4 witzige Sätze", "tip": "Lustiger Liebestipp", "emoji": "Emoji"}`,
    userPromptTpl: 'Berechne den Kompatibilitäts-Score von "{{guestName}}" und "{{partnerName}}" auf "{{eventTitle}}".',
    negativePrompt: null, temperature: 0.9, maxTokens: 300, strength: null,
  },
...
export async function seedDefaultPrompts() {
  ...
  const categoryMap: Record<string, PromptCategory> = {
    ...
    ai_dj: 'GAME',
    ai_meme: 'GAME',
    ai_superlatives: 'GAME',
    ai_photo_critic: 'GAME',
    ai_couple_match: 'GAME',
    ai_categorize: 'SUGGEST',
    ai_oldify: 'STYLE',
    ai_cartoon: 'STYLE',
    ai_style_pop: 'STYLE',
  };
  ...
  const nameMap: Record<string, string> = {
    ...
    ai_roast: 'AI Roast',
    celebrity_lookalike: 'Promi-Doppelgänger',
    persona_quiz: 'Persona Quiz',
    wedding_speech: 'Hochzeitsrede',
    ai_stories: 'Story Generator',
    ai_bingo: 'Foto-Bingo',
    ai_dj: 'AI DJ',
    ai_meme: 'Meme Generator',
    ai_superlatives: 'Party Awards',
    ai_photo_critic: 'Foto-Kritiker',
    ai_couple_match: 'Couple Match',
    ai_categorize: 'AI Foto-Kategorisierung',
