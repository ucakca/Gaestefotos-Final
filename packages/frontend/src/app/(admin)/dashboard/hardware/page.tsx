    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg bg-app-bg p-0.5">
            <button
              onClick={() => setViewMode('inventory')}
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                viewMode === 'inventory' ? 'bg-blue-500 text-white' : 'text-app-muted hover:text-app-fg'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${
                viewMode === 'calendar' ? 'bg-blue-500 text-white' : 'text-app-muted hover:text-app-fg'
              }`}
            >
        ...
        <>
          ...
          <div className="relative mb-4">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-app-border bg-app-card text-app-fg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
            ...
            {filteredItems.map(item => {
              ...
              return (
                <motion.div
                  ...
                  className="rounded-2xl border border-app-border bg-app-card shadow-sm p-4"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-semibold text-app-fg">{item.name}</h3>
                      <p className="text-xs text-app-muted">{TYPE_LABELS[item.type] || item.type}</p>
                    </div>
                  ...
                  {item.serialNumber && (
                    <p className="text-xs text-app-muted mb-2 font-mono">SN: {item.serialNumber}</p>
                  )}
                  {item.notes && (
                    <p className="text-xs text-app-muted mb-2">{item.notes}</p>
                  )}
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-app-border">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-app-bg text-app-muted transition-colors"
                      >
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-16">
              <Package className="w-12 h-12 text-app-muted mx-auto mb-3 opacity-30" />
              <p className="text-app-muted">Keine Hardware gefunden</p>
            </div>
          )}
        </>
      ) : (
        ...
        <div className="rounded-2xl border border-app-border bg-app-card shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-7 text-center">
            {['Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa', 'So'].map(d => (
              <div key={d} className="text-xs font-medium text-app-muted py-2 border-b border-app-border">
                {d}
              </div>
            ))}
            {calendarDays.map((day, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`min-h-[80px] p-1 border-b border-r border-app-border/50 ${
                    !day ? 'bg-app-bg/30' : ''
                  } ${isToday ? 'bg-blue-50' : ''}`}
                >
                  {day && (
                    <>
                      <div className={`text-xs font-medium mb-1 ${isToday ? 'text-blue-600 font-bold' : 'text-app-muted'}`}>
                        {day}
                      </div>
                      ...
                      {dayBookings.length > 3 && (
                        <div className="text-[10px] text-app-muted">+{dayBookings.length - 3} mehr</div>
                      )}
                    </>
                  )}
                </div>
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed inset-x-4 top-1/2 -translate-y-1/2 max-w-md mx-auto z-50 bg-app-card rounded-2xl shadow-2xl p-6"
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-lg text-app-fg">{item ? 'Hardware bearbeiten' : 'Hardware hinzufügen'}</h3>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-app-bg"><X className="w-5 h-5 text-app-muted" /></button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Name</label>
            <input value={name} onChange={e => setName(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" placeholder="Photo Booth Alpha" />
          </div>
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Typ</label>
            <select value={type} onChange={e => setType(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm">
              {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Seriennummer</label>
            <input value={serialNumber} onChange={e => setSerialNumber(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" placeholder="Optional" />
          </div>
          {item && (
            <div>
              <label className="block text-sm font-medium text-app-fg mb-1">Status</label>
              <select value={status} onChange={e => setStatus(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm">
                {Object.entries(STATUS_LABELS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
              </select>
            </div>
          )}
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Notizen</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" placeholder="Optional" />
          </div>
        </div>
        <div className="flex justify-end gap-2 mt-5">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm text-app-muted hover:bg-app-bg">Abbrechen</button>
  ...
  return (
    <>
      ...
      <motion.div
        ...
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.95 }}
        className="fixed inset-x-4 top-1/2 -translate-y-1/2 max-w-md mx-auto z-50 bg-app-card rounded-2xl shadow-2xl p-6 max-h-[85vh] overflow-y-auto"
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-lg text-app-fg">Neue Buchung</h3>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-app-bg"><X className="w-5 h-5 text-app-muted" /></button>
        </div>
        {error && (
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Hardware</label>
            <select value={hardwareId} onChange={e => setHardwareId(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm">
              {items.map(i => <option key={i.id} value={i.id}>{i.name} ({TYPE_LABELS[i.type] || i.type})</option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-app-fg mb-1">Von</label>
              <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-app-fg mb-1">Bis</label>
              <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Kundenname</label>
            <input value={customerName} onChange={e => setCustomerName(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" placeholder="Optional" />
          </div>
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">E-Mail</label>
            <input type="email" value={customerEmail} onChange={e => setCustomerEmail(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" placeholder="Optional" />
          </div>
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Preis (€)</label>
            <input type="number" value={totalPrice} onChange={e => setTotalPrice(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" placeholder="Optional" />
          </div>
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Notizen</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} className="w-full px-3 py-2 rounded-lg border border-app-border bg-app-bg text-app-fg text-sm" placeholder="Optional" />
