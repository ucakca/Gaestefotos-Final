    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/50">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">{error || 'Partner nicht gefunden'}</p>
              ...
              {[
                ...
                <div key={stat.label} className="bg-card rounded-xl border p-5">
                  ...
                  <div className="text-2xl font-bold text-foreground">
                    {stat.value}
                    {stat.max && (
                      <span className="text-sm font-normal text-muted-foreground/70 ml-1">/ {stat.max}</span>
                    )}
                  </div>
                </div>
              ))}
            ...
            <div className="lg:col-span-2">
              <div className="bg-card rounded-xl border">
                <div className="p-4 border-b flex items-center justify-between">
                  <h2 className="font-semibold text-foreground">Partner-Events ({events.length})</h2>
                </div>
                {events.length === 0 ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card rounded-xl border p-4">
                ...
                {(!partner.hardware || partner.hardware.length === 0) ? (
                  <p className="text-sm text-muted-foreground/70">Keine Hardware registriert</p>
                ) : (
                  <div className="space-y-2">
                    {partner.hardware.map((hw: any) => (
                      <div key={hw.id} className="flex items-center justify-between py-1.5">
                        <div className="text-sm">
                          <span className="font-medium text-foreground/80">{hw.name}</span>
                          <span className="text-muted-foreground/70 text-xs ml-1">{hw.type}</span>
                        </div>
              ...
              <div className="bg-card rounded-xl border p-4">
                ...
                {(!partner.members || partner.members.length === 0) ? (
                  <p className="text-sm text-muted-foreground/70">Keine Mitglieder</p>
                ) : (
                  <div className="space-y-2">
                    {partner.members.map((m: any) => (
                      <div key={m.id} className="flex items-center justify-between py-1.5">
                        <div className="text-sm">
                          <span className="font-medium text-foreground/80">{m.user?.name}</span>
                          <span className="text-muted-foreground/70 text-xs ml-1">{m.user?.email}</span>
                        </div>
                        <span className="text-xs px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">{m.role}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="bg-card rounded-xl border p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-foreground/80 font-mono text-xs">{partner.primaryColor || '#6366f1'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-foreground/80 font-mono text-xs">{partner.accentColor || '#10b981'}</span>
                    </div>
                    {partner.customDomain && (
                      <div className="flex items-center gap-2">
                        <span className="text-muted-foreground">Domain:</span>
                        <span className="text-foreground/80">{partner.customDomain}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-card rounded-xl border p-4">
                  ...
                  <div className="space-y-2">
                    {billingPeriods.slice(0, 5).map((bp: any) => {
                      ...
                      return (
                        <div key={bp.id} className="p-2.5 bg-muted/50 rounded-lg border">
              ...
              <div className="bg-card rounded-xl border p-4">
                ...
                {subscriptions.length === 0 ? (
                  <p className="text-sm text-muted-foreground/70">Kein aktives Abo</p>
                ) : (
                  <div className="space-y-3">
                    {subscriptions.map((sub) => {
                      ...
                      return (
                        <div key={sub.id} className="p-3 bg-muted/50 rounded-lg border">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <Package className="w-4 h-4 text-indigo-500" />
                              <span className="text-sm font-semibold text-foreground">{sub.plan}</span>
                              <span className="text-xs text-muted-foreground/70">{sub.interval === 'YEARLY' ? 'Jährlich' : 'Monatlich'}</span>
                            </div>
                            <span className={`text-xs px-1.5 py-0.5 rounded-full ${statusColors[sub.status] || 'bg-muted text-muted-foreground'}`}>
                              {sub.status}
                            </span>
                          </div>
                            ...
                            <div className="space-y-1 mb-2">
                              {sub.devices.filter(d => d.isActive).map((d) => (
                                <div key={d.id} className="flex items-center justify-between text-xs">
                                  <span className="text-muted-foreground">{d.deviceType.replace(/_/g, ' ')}</span>
                                  <span className="text-muted-foreground">{(d.pricePerMonthCents / 100).toFixed(2)} €/Mo</span>
                                </div>
                              ))}
                            </div>
                          )}
                          <div className="flex items-center justify-between pt-2 border-t border-border">
                            <span className="text-xs text-muted-foreground">Gesamt/Monat</span>
              ...
              <div className="bg-muted rounded-xl p-4 text-xs text-muted-foreground">
                <div className="flex justify-between mb-1">
                  ...
                  <span className="font-medium text-foreground/80">{partner.maxStorageGb} GB</span>
                </div>
                <div className="flex justify-between mb-1">
                  <span>Event-Limit:</span>
                  <span className="font-medium text-foreground/80">{partner.maxEvents}</span>
                </div>
                <div className="flex justify-between">
                  <span>Provision:</span>
                  <span className="font-medium text-foreground/80">{partner.commissionPct}%</span>
                </div>
              </div>
            </div>
          </div>
