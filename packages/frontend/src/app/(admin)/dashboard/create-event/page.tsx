...
export default function AdminCreateEventPage() {
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-12">
        <motion.div
          ...
          className="rounded-2xl bg-app-card border border-app-border shadow-lg p-8 text-center"
        >
          ...
          <p className="text-xs text-app-muted mb-6 font-mono bg-app-bg rounded-lg px-3 py-2">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <button
              onClick={resetForm}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-app-bg text-app-fg text-sm font-medium hover:bg-app-hover transition-colors"
            >
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      {/* Step 1: User Selection */}
      <div className="rounded-2xl border border-app-border bg-app-card shadow-sm p-5 mb-4">
        ...
        {selectedUser ? (
          <div className="flex items-center gap-3 p-3 rounded-xl bg-blue-50 border border-blue-200">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-medium text-sm text-app-fg">{selectedUser.name}</div>
          ...
          <div className="space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-app-border bg-app-bg text-app-fg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                autoFocus
              />
              {searchLoading && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-app-muted animate-spin" />}
            </div>
            <AnimatePresence>
              {users.length > 0 && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -5 }}
                  className="rounded-xl border border-app-border bg-app-card shadow-lg overflow-hidden max-h-60 overflow-y-auto"
                >
                  {users.map(u => (
                    <button
                      key={u.id}
                      onClick={() => { setSelectedUser(u); setUserSearch(''); setUsers([]); }}
                      className="flex items-center gap-3 w-full px-3 py-2.5 hover:bg-app-hover transition-colors text-left border-b border-app-border/50 last:border-0"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-app-fg truncate">{u.name}</div>
      ...
      {/* Step 2: Event Details */}
      <div className={`rounded-2xl border border-app-border bg-app-card shadow-sm p-5 mb-4 transition-opacity ${selectedUser ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
        <h2 className="font-semibold text-app-fg mb-3 flex items-center gap-2">
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-app-border bg-app-bg text-app-fg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="flex items-center gap-1 text-sm font-medium text-app-fg mb-1">
                <Calendar className="w-3.5 h-3.5 text-app-muted" /> Datum
              </label>
              <input
                type="datetime-local"
                value={dateTime}
            ...
            <div>
              <label className="flex items-center gap-1 text-sm font-medium text-app-fg mb-1">
                <MapPin className="w-3.5 h-3.5 text-app-muted" /> Ort
              </label>
              <input
                value={locationName}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="flex items-center gap-1 text-sm font-medium text-app-fg mb-1">
                <Lock className="w-3.5 h-3.5 text-app-muted" /> Passwort
              </label>
              <input
                value={password}
            ...
            <div>
              <label className="flex items-center gap-1 text-sm font-medium text-app-fg mb-1">
                <Package className="w-3.5 h-3.5 text-app-muted" /> Paket
              </label>
              <select
                value={packageSku}
          ...
          <div>
            <label className="block text-sm font-medium text-app-fg mb-1">Admin-Notizen</label>
            <textarea
              value={notes}
              onChange={e => setNotes(e.target.value)}
              rows={2}
              placeholder="Interne Notizen (sichtbar als Profil-Beschreibung)"
              className="w-full px-3 py-2.5 rounded-xl border border-app-border bg-app-bg text-app-fg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
            />
          </div>
        </div>
      </div>
