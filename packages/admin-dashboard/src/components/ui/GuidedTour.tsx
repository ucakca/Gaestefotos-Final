...
export default function GuidedTour({ tourId, steps, autoStart = true }: GuidedTourProps) {
  ...
  const bubbleStyle: CSSProperties | undefined = useMemo(() => {
      ...
      >
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <Button type="button" variant="outline" size="sm" onClick={prev} disabled={idx === 0}>
              Zurück
            </Button>
            {idx < steps.length - 1 ? (
        ...
        {!pos ? (
          <div className="mt-2 text-xs text-[var(--status-warning)]">
            Hinweis: Ziel-Element nicht gefunden. Bitte scrolle etwas.
          </div>
        ) : null}
      </div>
    </>
