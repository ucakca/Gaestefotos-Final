...
const upload = multer({
  ...
  limits: {
    fileSize: 100 * 1024 * 1024, // 100MB (aligned with TUS limit)
  },
...
const uploadSinglePhoto = (req: AuthRequest, res: Response, next: any) => {
  upload.single('file')(req as any, res as any, (err: any) => {
    ...
    if (code === 'LIMIT_FILE_SIZE') {
      return res.status(400).json({ error: 'Datei zu groß. Maximum: 100MB' });
    }
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!photo) return res.status(404).json({ error: 'Foto nicht gefunden' });
      if (!(await hasEventManageAccess(req, photo.eventId))) return res.status(403).json({ error: 'Zugriff verweigert' });
...
// ─── Sub-Routers (extracted from monolithic photos.ts) ─────────────────────
import photoStatsRouter from './photoStats';
import photoBulkRouter from './photoBulk';
import photoDownloadRouter from './photoDownload';

router.use(photoStatsRouter);
router.use(photoBulkRouter);
router.use(photoDownloadRouter);
